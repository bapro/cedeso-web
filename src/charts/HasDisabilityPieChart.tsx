...
interface HasDisabilityPieChartProps {
  apiEndpoint: string;
}

const HasDisabilityPieChart: React.FC<HasDisabilityPieChartProps> = ({
  apiEndpoint,
}) => {
    ...
    const fetchData = async () => {
      try {
        ...
        data.forEach((item: any) => {
          ...
          if (capture && capture.hasDisability) {
            disabilityCounts[capture.hasDisability] =
              (disabilityCounts[capture.hasDisability] || 0) + 1;
          }
        });
        // --- DATA TRANSFORMATION LOGIC END ---

        const labels = Object.keys(disabilityCounts);
        const dataValues = Object.values(disabilityCounts);
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: false,
        text: "¿Tiene alguna discapacidad?", // Translated
      },
      tooltip: {
        callbacks: {
  ...
  return (
    <div className="pie-chart-container">
      <h1 className="chart-title">¿Tiene alguna discapacidad?</h1>
...
export default HasDisabilityPieChart;
