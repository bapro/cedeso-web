...
interface IsLGBTQPieChartProps {
  apiEndpoint: string;
}

const IsLGBTQPieChart: React.FC<IsLGBTQPieChartProps> = ({ apiEndpoint }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const lgbtqCounts: { [key: string]: number } = {};

        data.forEach((item: any) => {
          const capture = item.capture; // Access the capture object

          if (capture && capture.isLGBTQ) {
            lgbtqCounts[capture.isLGBTQ] =
              (lgbtqCounts[capture.isLGBTQ] || 0) + 1;
          }
        });
        // --- DATA TRANSFORMATION LOGIC END ---

        const labels = Object.keys(lgbtqCounts);
        const dataValues = Object.values(lgbtqCounts);
  ...
  return (
    <div className="pie-chart-container">
      <h1 className="chart-title">Es LGBTQ+</h1>
...
export default IsLGBTQPieChart;
