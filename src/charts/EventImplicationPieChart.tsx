...
interface EventImplicationPieChartProps {
  apiEndpoint: string;
}

const EventImplicationPieChart: React.FC<EventImplicationPieChartProps> = ({
  apiEndpoint,
}) => {
  const [chartData, setChartData] = useState<{
    ...
    datasets: [
      {
        label: "Tipo de Evento", // Translated
        data: [],
        backgroundColor: [],
        borderColor: [],
        borderWidth: 1,
      ...
      try {
        ...
        data.forEach((item: any) => {
          ...
          if (
            capture &&
            capture.eventType &&
            Array.isArray(capture.eventType)
          ) {
            capture.eventType.forEach((event: string | number) => {
        ...
        setChartData({
          ...
          datasets: [
            {
              label: "Tipos de Eventos", // Translated
              data: dataValues,
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: false,
        text: "Tipo de Evento", // Translated
      },
      tooltip: {
        callbacks: {
  ...
  return (
    <div className="pie-chart-container">
      <h1 className="chart-title">¿En qué consistió la operación?</h1>
...
export default EventImplicationPieChart;
