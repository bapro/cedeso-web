...
interface AuthorityVehiclesPieChartProps {
  apiEndpoint: string;
}

const AuthorityVehiclesPieChart: React.FC<AuthorityVehiclesPieChartProps> = ({
  apiEndpoint,
}) => {
  const [chartData, setChartData] = useState<{
    ...
    datasets: [
      {
        label: "Vehículos de Autoridad", // Translated
        data: [],
        backgroundColor: [],
        borderColor: [],
        borderWidth: 1,
    ...
    const fetchData = async () => {
      try {
        ...
        data.forEach((item: any) => {
          ...
          if (
            capture &&
            capture.authorityVehicles &&
            Array.isArray(capture.authorityVehicles)
          ) {
            capture.authorityVehicles.forEach((vehicle: string | number) => {
        ...
        const dataValues = Object.values(vehicleCounts);
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: false,
        text: "Identificación de vehículos-s de la autoridad hizo el evento", // Translated
      },
      tooltip: {
        callbacks: {
  ...
  return (
    <div className="pie-chart-container">
      <h1 className="chart-title">
        Identificación de vehículos-s de la autoridad hizo el evento
      </h1>
...
export default AuthorityVehiclesPieChart;
